...
let currentPlayer = PieceColor.WHITE; // Les blancs commencent toujours
let gameState = 'playing'; // Contrôle du statut de jeu
let hasMoved = false; // Indique si un mouvement a déjà été effectué dans ce tour
let capturedWhite = []; // Liste des pièces capturées par les Blancs
let capturedBlack = []; // Liste des pièces capturées par les Noirs
...
export function handleMove(fromX, fromY, toX, toY) {
    ...
    if (piece.isValidMove(fromX, fromY, toX, toY, board)) {
        ...
        if (board.movePiece(fromX, fromY, toX, toY)) {
            ...
            const opponentColor = currentPlayer === PieceColor.WHITE
                ? PieceColor.BLACK
                : PieceColor.WHITE;
